...
use trace::{Batch, BatchReader, Batcher, Cursor, Trace, TraceReader};
use {AsCollection, Collection, Data, ExchangeData, Hashable};
...
use trace::wrappers::filter::{BatchFilter, TraceFilter};
    ...
    Tr: TraceReader<Time = G::Timestamp> + Clone,
{
        ...
        F: FnMut(&Tr::Key, &Tr::Val) -> bool + Clone + 'static,
    {
        ...
        Arranged {
            ...
            stream: self
                .stream
                .map(move |bw| BatchFilter::make_from(bw, logic2.clone())),
        }
    }
    ...
    ) -> Stream<G, (Tr::Key, Tr::Val, G::Timestamp, Tr::R)>
    where
        ...
        Tr: 'static,
    {
        ...
        let exchange =
            Exchange::new(move |update: &(Tr::Key, G::Timestamp)| update.0.hashed().into());
        queries.binary_frontier(
            &self.stream,
            exchange,
            Pipeline,
            "TraceQuery",
                ...
                move |input1, input2, output| {
                    input1.for_each(|time, data| {
                        ...
                        if capability.is_none()
                            || time.time().less_than(capability.as_ref().unwrap().time())
                        {
                                ...
                                if cursor.get_key(&storage) == Some(key) {
                                    ...
                                    while let Some(val) = cursor.get_val(&storage) {
                                        cursor.map_times(&storage, |t, d| {
                                            working.push((t.clone(), val.clone(), d.clone()))
                                        });
                                        cursor.step_val(&storage);
                                    }
                    ...
                    if let Some(frontier) = frontier {
                        trace
                            .as_mut()
                            .map(|t| t.set_logical_compaction(AntichainRef::new(&[frontier])));
                    } else {
                        trace = None;
                    }
                }
    ...
    R: Semigroup,
{
        ...
        Tr::Batch: Batch,
    {
        ...
        let stream = {
            ...
            self.inner
                .unary_frontier(pact, name, move |_capability, info| {
                    ...
                    let (activator, effort) = if let Some(effort) = self
                        .inner
                        .scope()
                        .config()
                        ...
                        if prev_frontier.borrow() != input.frontier().frontier() {
                                ...
                                .any(|c| !input.frontier().less_equal(c.time()))
                            {
                                ...
                                for (index, capability) in
                                    capabilities.elements().iter().enumerate()
                                {
                                    if !input.frontier().less_equal(capability.time()) {
                                        ...
                                        for other_capability in
                                            &capabilities.elements()[(index + 1)..]
                                        {
                                        ...
                                        writer
                                            .insert(batch.clone(), Some(capability.time().clone()));
    ...
    G::Timestamp: Lattice + Ord,
{
    ...
    where
        ...
        Tr::Batch: Batch,
    {
        self.map(|k| (k, ())).arrange_core(pact, name)
    }
}
...
impl<G: Scope, K: ExchangeData + Hashable, R: ExchangeData + Semigroup> ArrangeBySelf<G, K, R>
    ...
    G::Timestamp: Lattice + Ord,
{
    ...
    fn arrange_by_self_named(
        ...
        self.map(|k| (k, ())).arrange_named(name)
    }
}
