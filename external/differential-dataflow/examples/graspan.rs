...
impl<'a> From<&'a str> for Production {
    fn from(text: &'a str) -> Production {
        ...
        let left_hand = names
            .next()
            .expect("All rules must have a target.")
            .to_string();
        ...
        let relations = names
            .map(|name| {
                if name.starts_with('~') {
                    Relation::Reverse(name.split_at(1).1.to_string())
                } else {
                    Relation::Forward(name.to_string())
                }
            })
            .collect();
...
pub struct EdgeVariable<G: Scope>
where
    G::Timestamp: Lattice,
{
...
impl Query {
    ...
    pub fn build_from<'a>(iterator: impl IntoIterator<Item = &'a str>) -> Self {
        Query {
            productions: iterator
                .into_iter()
                .map(|text| Production::from(text))
                .collect(),
        }
    }

    /// Creates a dataflow implementing the query, and returns input and trace handles.
    pub fn render_in<G: Scope>(
        &self,
        scope: &mut G,
    ) -> IndexMap<String, RelationHandles<G::Timestamp>>
    where
        G::Timestamp: Lattice + ::timely::order::TotalOrder,
    {
        ...
        for production in self.productions.iter() {
            input_map
                .entry(production.left_hand.clone())
        ...
        scope.iterative::<Iter, _, _>(|subscope| {
            ...
            for (name, (input, collection)) in input_map.drain(..) {
                let edge_variable = EdgeVariable::from(
                    &collection.enter(subscope),
            ...
            for production in self.productions.iter() {
                ...
                if rule.len() > 0 {
                    ...
                    variable_map[name]
                        .add_production(&transposed.as_collection(|&dst, &src| (src, dst)));
                }
            }
...
fn main() {
    timely::execute_from_args(std::env::args(), move |worker| {
        ...
        let query_filename = std::env::args()
            .nth(1)
            .expect("Argument 1 (query filename) missing.");
        let query_text =
            std::fs::read_to_string(query_filename).expect("Failed to read query file");
        let query = Query::build_from(query_text.lines());

        let mut relation_map = worker.dataflow::<(), _, _>(|scope| query.render_in(scope));

        if index == 0 {
            println!("{:?}:\tDataflow assembled for {:?}", timer.elapsed(), query);
        }
        ...
        let data_filename = std::env::args()
            .nth(2)
            .expect("Argument 2 (data filename) missing.");
        ...
        for readline in file.lines() {
            ...
            if !line.starts_with('#') && line.len() > 0 {
                ...
                let src = elts
                    .next()
                    .expect("data line with no src (1st) element")
                    .parse()
                    .expect("malformed src");
                if (src as usize) % peers == index {
                    let dst = elts
                        .next()
                        .expect("data line with no dst (2nd) element")
                        .parse()
                        .expect("malformed dst");
