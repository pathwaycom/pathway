    ...
    G::Timestamp: Lattice + Hash + Ord,
{
    ...
    let reverse = edges
        .map_in_place(|x| ::std::mem::swap(&mut x.0, &mut x.1))
        .arrange_by_key();
    ...
    for &(src, dst) in relations.iter() {
        ...
        results = match (src_present, dst_present) {
            ...
            (true, false) => {
                ...
                results.map(move |vec| (vec[src], vec)).join_core(
                    &forward,
                    move |_src_val, vec, &dst_val| {
            ...
            (false, true) => {
                ...
                results.map(move |vec| (vec[dst], vec)).join_core(
                    &reverse,
                    move |_dst_val, vec, &src_val| {
                        let mut temp = vec.clone();
                        while temp.len() <= src {
                            temp.push(0);
                        }
