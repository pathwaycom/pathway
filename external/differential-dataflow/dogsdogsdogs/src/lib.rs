...
impl<G: Scope, R: Monoid + Multiply<Output = R>, P, E> ValidateExtensionMethod<G, R, P, E>
    for Collection<G, (P, E), R>
{
    fn validate_using<PE: PrefixExtender<G, R, Prefix = P, Extension = E>>(
        &self,
        extender: &mut PE,
    ...
    R: Monoid + Multiply<Output = R> + ExchangeData,
{
    pub fn index<G: Scope<Timestamp = T>>(collection: &Collection<G, (K, V), R>) -> Self {
        ...
        let counts = arranged.distinct().map(|(k, _v)| k).arrange_by_self().trace;
    ...
    pub fn extend_using<P, F: Fn(&P) -> K + Clone>(
        &self,
        logic: F,
    ) -> CollectionExtender<K, V, T, R, P, F> {
        CollectionExtender {
