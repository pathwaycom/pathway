...
Introducing a Kafka-free architecture using [Pathway Live Data Framework](/framework) ([GitHub repo](https://github.com/pathwaycom/pathway)) alongside Delta Tables on S3-compatible storage, such as [MinIO](https://min.io/). The Pathway Live Data Framework is a stream processing engine with a Rust backend that allows you to build efficient, real-time data pipelines in Python for both on-premise and cloud environments (AWS, Google Cloud, Azure). In this architecture, Pathway Live Data Framework manages the reading and writing of data to and from Delta Tables, creating a seamless, scalable, and cost-effective streaming solution that leverages your existing S3 storage—eliminating the need for Kafka.
...
When using the Pathway Live Data Framework in conjunction with Delta Lake and MinIO, you will:
...
**The Pathway Live Data Framework provides the streaming capabilities that Delta Tables alone lack: it continuously monitors, reads, and processes new data entries and updates onto the Delta Tables.**
By handling the reading and writing of data to and from these Delta Tables, the Pathway Live Data Framework provides an easy-to-deploy and efficient way to process data streams on S3.

With the Pathway Live Data Framework in integration with your existing S3 storage, you can handle up to 60,000 messages per second with 0.7s (p99) latency, and even 250k messages per second with near-real-time latency.
...
Using the Pathway Live Data Framework with S3-compatible storage like MinIO allows you to create a streaming architecture that is both effective and straightforward.
Unlike traditional Kafka setups, which involve managing clusters, brokers, and zookeepers, a Pathway Live Data Framework-based solution simplifies your infrastructure by relying on your existing S3 storage.
...
3. **Pathway Live Data Framework**: The Pathway Live Data Framework serves as the processing engine that seamlessly reads and writes data to and from Delta Tables, enabling streamlined management of data streams.
...
- `etl.py` is the main Pathway Live Data Framework ETL pipeline, handling data loading and writing to Delta Tables on MinIO.
...
Bravo! You’ve successfully built a streaming pipeline on MinIO using the Pathway Live Data Framework and Delta Tables—an effective alternative to traditional messaging systems like Kafka.
::


## Benchmarks for this S3 Streaming Setup without Kafka

Pathway is often used by developers for ultra-low latency use cases, such as [Formula 1 racing analytics](/solutions/ai-enabled-vehicles-n-esports) and mission-critical applications for organizations like [Intel](/framework/blog/intel-summit) and [NATO](https://euro-sd.com/2024/10/major-news/40613/nato-tests-pathway-rest/).
...
The benchmark experiments below are to see if the Pathway Live Data Framework performs as a Python Kafka alternative with your existing S3-compatible storage.

### Benchmark Setup:

In these tests, messages are written to a Delta Table hosted on MinIO (a popular S3-compatible storage system) and then read back. Latency is measured from the moment a message is sent to when it's successfully processed by the Pathway Live Data Framework.
...
The Pathway Live Data Framework integrates seamlessly with AWS S3, providing a powerful Kafka alternative for AWS users.
Available on the [AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-l6jeey6e3rqqk), it simplifies deployment within your AWS environment.

To set up the Pathway Live Data Framework with AWS S3, deploy it from the AWS Marketplace and configure it with your AWS credentials.
Use the Delta Lake connectors to read from and write to Delta Tables stored in S3.
...
The Pathway Live Data Framework serves as an efficient Kafka alternative on the Google Cloud Platform (GCP).
By deploying it on GCP, you can build streaming pipelines without the need to manage Kafka clusters.

To set up the Pathway Live Data Framework on Google Cloud, deploy it using [Google Kubernetes Engine (GKE)](https://cloud.google.com/kubernetes-engine).
Configure it to interact with Google Cloud Storage, enabling you to process streaming data effectively.
This setup leverages Google's scalable infrastructure without the complexity of Kafka.

You can read more about deploying the Pathway Live Data Framework on GCP in our [GCP deployment guide](/developers/user-guide/deployment/gcp-deploy).

## Azure Kafka Alternative
The Pathway Live Data Framework offers a compelling Kafka alternative for Microsoft Azure users.
Deploying it on Azure allows you to build streaming pipelines without the overhead of managing Kafka infrastructure.

To set up the Pathway Live Data Framework on Azure, deploy it using [Azure Container Instances](https://github.com/pathwaycom/pathway/tree/main/examples/projects/azure-aci-deploy).
Configure it to interact with Azure storage services, enabling you to process streaming data efficiently.
This setup utilizes Azure's scalable infrastructure without the complexity of Kafka.

You can read more about deploying the Pathway Live Data Framework on Azure in our [Azure deployment guide](/developers/user-guide/deployment/azure-aci-deploy).
