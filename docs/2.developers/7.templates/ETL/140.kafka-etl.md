...
description: "Learn how to build a Kafka ETL pipeline in Python with the Pathway Live Data Framework and process event streams in real-time."
author: 'olivier'
layout: "template"
date: '2024-02-15'
...
In this tutorial, you will learn how to write a Kafka ETL in Python using Pathway Live Data Framework, an open-source event processing framework.
You will use Pathway Live Data Framework connectors and transformations to extract, transform, and load event streams across multiple Kafka topics.
...
The Pathway Live Data Framework, a Python stream processing framework with a Rust engine, makes building ETL pipeline over Kafka in pure Python simple without compromising on performance.
...
More precisely, you will learn how to do the different ETL steps with the Pathway Live Data Framework:
- (E) extracting different data streams from Kafka using the Kafka input connector,
- (T) converting the times with varying time zones into timestamps using the datetime module,
- (T) concatenating the resulting data streams using the concatenation function,
- (L) loading the final data stream back into Kafka.

The full pipeline script is [here](#full-solution) for those in a hurry.
...
![Kafka ETL with Pathway Live Data Framework](/assets/content/showcases/ETL-Kafka/ETL-Kafka.svg)
...
## ETL with Pathway Live Data Framework: Concatenating data streams

Now that the setup is ready, the Pathway Live Data Framework can manage all the rest!

### Extract from Kafka
The Pathway Live Data Framework provides [connectors](/developers/user-guide/connect/live-data-framework-connectors) to connect and extract data from different data sources.
You can connect to Kafka using the Kafka connectors.

In the Pathway Live Data Framework, data is represented using [tables](/developers/user-guide/introduction/concepts#tables-dynamic-content-with-static-schema) and you need to define the data types of the extracted data using [schemas](/developers/user-guide/connect/schema):
...
You can learn more about the Kafka connectors in our [dedicated tutorial](/developers/user-guide/connect/connectors/kafka_connectors).
...
Fortunately, the Pathway Live Data Framework provides all the datetime functions to make it easy.

Here is how to concatenate the datetime from different time zones into a single data stream with timestamps with the Pathway Live Data Framework:
...
This is a very simple example of what the Pathway Live Data Framework can do.
The Pathway Live Data Framework supports more complex operations such as stateful ([groupby](/developers/user-guide/data-transformation/groupby-reduce-manual), [windows](/developers/user-guide/temporal-data/windows-manual), etc.) and temporal (e.g., [ASOF join](/developers/user-guide/temporal-data/asof-join)) operations.
...
No data is actually loaded into the system so if you were to run your code now, the pipeline would be built, but there would be no computation.
...
Thanks to its powerful Rust engine, [Pathway Live Data Framework computations are extremely fast](/framework/blog/streaming-benchmarks-pathway-fastest-engine-on-the-market).
You are not bound by the usual limits of Python, the Pathway Live Data Framework natively supports multithreading, multiprocessing and distributed computing.
...
You can learn more about the output [here](/developers/user-guide/introduction/first-realtime-app#interpret-the-output).
...
## Going further with Pathway Live Data Framework
Congratulations!
You are now ready to do Kafka ETL with the Pathway Live Data Framework.

Your setup probably differs slightly, and your ETL pipeline may require different operators.
The Pathway Live Data Framework offers many [connectors](/developers/user-guide/connect/supported-data-sources) for extracting and loading the data from and to various sources.
In addition to standard [table operations](/developers/user-guide/data-transformation/table-operations), the Pathway Live Data Framework also supports temporal operations such as [ASOF joins](/developers/user-guide/temporal-data/asof-join) and [interval joins](/developers/user-guide/temporal-data/interval-join).
<!--You can also define your own [user-defined functions](). WAITING FOR ARTICLE-->
Don't hesitate to take a look at [Pathway Live Data Framework documentation](/developers/user-guide/introduction/welcome) and reach out to us on [Discord](https://discord.com/invite/pathway) if you don't find the operator you are looking for.
