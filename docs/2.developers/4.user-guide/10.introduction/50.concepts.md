---
title: 'Core Concepts'
description: 'A review of the core concepts behind the Pathway Live Data Framework.'
---


# Core Concepts
A review of the core concepts behind data representation and data transformation in the programming layer of the Pathway Live Data Framework.

These are the core concepts you need to know about the framework:
...
In the Pathway Live Data Framework, there is a clear divide between the definition of the pipeline and the execution of the computation.
...
**Connectors** are the framework's interface with external systems, for both extracting the input data and sending the output data.
...
Input connectors return a table which represents a snapshot of the input stream (more information on tables [below](#tables-dynamic-content-with-static-schema)).
...
The Pathway Live Data Framework comes with many input connectors:
...
**In the Pathway Live Data Framework, data is modeled as tables, representing snapshots of the data streams.**
...
As Pathway Live Data Framework handles potentially infinite and ever-changing streaming data, the number of rows and the content changes with time as new data comes into the system.

### Processing the data with transformations

The Pathway Live Data Framework provides operators to modify the tables such as `select` or `join`.
These operators are called *transformations*.
The framework has a functional programming approach: each transformation returns a new table, leaving the input table unchanged.
...
Here is a way to do it with the Pathway Live Data Framework, assuming a correct input table called `input_table`:
...
Don't hesitate to read our article about [Pathway Live Data Framework basic transformations](/developers/user-guide/data-transformation/table-operations).

#### External functions and LLMs

The Pathway Live Data Framework provides many ready-to-use transformations, but it may not be enough for your project.
If you don't find what you need, don't worry you can use any Python function in your pipeline.
**Pathway Live Data Framework allows you to seamlessly integrate with Python Machine Learning libraries, use LLM's, and call into synchronous and asynchronous API's.**


### Send the results to external systems using output connectors.

The data is sent out of the Pathway Live Data Framework using **output connectors**.
...
The Pathway Live Data Framework comes with many output connectors, you can learn more about them in [our dedicated article](/developers/user-guide/connect/live-data-framework-connectors).
...
The tables you want to send out of the framework to external systems are also dynamic.
...
**Pathway Live Data Framework handles the data in an incremental way.**
Instead of sending the entire version of the table whenever there is an update, the framework only sends the changes to the table to be more efficient.
Only the rows that are affected by the changes are sent to the external system.

For example, consider the case of a pipeline computing the sum of the ages.
...
The Pathway Live Data Framework output connectors adapt the updates to match the system constraints.
...
In the Pathway Live Data Framework, the processing pipeline is modeled using a graph.
...
For example, the previous pipeline is represented as follows:
...
This dataflow is the core of the Pathway Live Data Framework.
The user creates a pipeline which is translated into a dataflow by the framework.
The graph is built by the calls to operators but, at that point, no computations are done: there is simply no data.
...
The Pathway Live Data Framework listens to the data sources for new updates until the process is terminated: **the computation runs forever until the process gets killed**.
**This is the normal behavior of the framework**.
...
This dataflow allows the user to focus on the intended behavior of the processing pipeline, as if the data were static, and the framework handles the updates on its own using the dataflow.

### Fast in-memory computations thanks to a powerful Rust Engine

In the Pathway Live Data Framework, both the storage of the tables and the computations are done in memory.
...
Fortunately, the framework comes with a powerful Rust engine which takes over once the pipeline is ready.
Python is used for its accessibility to describe the (typed) pipeline, but the dataflow is built and maintained by the Pathway Live Data Framework engine.
The engine removes those limits associated with Python.
In particular, **Pathway Live Data Framework natively supports multithreading and multiprocessing, and can be distributed using Kubernetes.**
The content of the tables is handled by the Rust engine, making it very memory-efficient.
Similarly, most of the transformations are handled at the Rust level, making the processing very fast.

If you add the incremental nature of the computations, you end-up with the [fastest data processing engine on the market](/framework/blog/streaming-benchmarks-pathway-fastest-engine-on-the-market).


### Static mode

With Pathway Live Data Framework, it doesn't matter if you are dealing with static or streaming data.
The same pipeline can be used for both kinds of data, and the engine provides consistent outputs in both cases.
...
The only time Pathway Live Data Framework is aware of is when the record arrives to the engine.
...
The Pathway Live Data Framework remembers old values through its input connectors, even when only stateless operations are used.
