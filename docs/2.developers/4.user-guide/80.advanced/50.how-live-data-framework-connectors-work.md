---
title: 'How Pathway Live Data Framework Connectors Work'
description: 'This page describes Pathway Live Data Framework connectors architecture and their capabilities'
---

# How Pathway Live Data Framework Connectors Work

The Pathway Live Data Framework enables you to efficiently ingest data from a wide variety of sources, such as task queues (e.g., [Kafka](/developers/user-guide/connect/connectors/kafka_connectors/), [NATS](/developers/user-guide/connect/connectors/nats-connectors/), [MQTT](/developers/api-docs/pathway-io/mqtt/), [Kinesis](/developers/api-docs/pathway-io/kinesis/)), [file systems](/developers/user-guide/connect/connectors/fs-connector/), and databases. This data is generally processed efficiently thanks to the runtime being implemented in Rust.
...
This article is intended for anyone interested in the design of Pathway Live Data Framework connectors, those considering contributing to the connector ecosystem, and anyone curious about the internal architecture of the framework in general.

You will learn:

* How Pathway Live Data Framework abstracts connector functionality,
* The basic interfaces that need to be implemented on the Rust engine side to create a custom connector and contribute it to Pathway, and
...
Before thinking about the architecture itself, it's important to understand the general properties that a data processing framework such as Pathway Live Data Framework should satisfy. This ensures that the architectural decisions are justified.
...
With these foundational considerations and input data mechanisms in place, the next section will describe the minimal interface — or _trait_, in Rust terminology — that must be implemented to create a custom input connector for Pathway Live Data Framework.
...
Above is described a simplified, yet fully functional architecture of input connectors in Pathway Live Data Framework.
...
![The full cycle of reading in Pathway Live Data Framework](assets/content/documentation/connectors/input_connector.svg)
...
For completeness, it is worth describing how output connectors are implemented in Pathway Live Data Framework. Note that this process is completely independent from input connectors: you may implement only the input part or only the output part for a given system. They do not depend on each other.
...
![The full cycle of output in Pathway Live Data Framework](assets/content/documentation/connectors/output_connector.svg)

In brief, you can turn a buffering Rust wrapper over an external system into a functional and performant connector.

## Python Bridge

Connectors in Pathway Live Data Framework are written in **Rust**, as is the Pathway Live Data Framework runtime. This raises the question: how are these Rust connectors exposed to Python?
...
A Pathway Live Data Framework pipeline consists of several parts, and for the purposes of this article two of them are relevant:
...
Before discussing performance benchmarks, it is important to note that input performance depends on how the data source is configured. If the upstream system is slow, performance can be partially improved through Pathway Live Data Framework's internal algorithms, but the bottleneck may still reside in the storage itself.

To get an estimate for an I/O-bound problem, you can refer to Pathway performance [benchmarks](/framework/blog/streaming-benchmarks-pathway-fastest-engine-on-the-market/) that were done to compare the framework to other stream-processing systems such as Spark Streaming and Flink. One of the benchmark scenarios was the classical **WordCount** task, which measures the frequency of word occurrences. The article also describes the setup, the hardware used and the methodology for an end-to-end latency measurement.
...
The Pathway Live Data Framework provides services and tools for observability, allowing users to monitor how the system operates and detect issues or failures. In particular, dashboards exist over [Grafana and OpenTelemetry](/developers/user-guide/deployment/live-data-framework-monitoring/) combination, and browser-based tools are available for monitoring computational graph as a whole and its performance over time.
...
You can learn how to use persistence in Pathway Live Data Framework [here](/developers/user-guide/deployment/persistence/).
...
At this point, you have a clear understanding of how connectors are integrated into Pathway Live Data Framework and how the platform handles them. The information from the first part of this overview must already be enough for you to write and contribute your own native connector, if you want to.
