...
description: 'This page describes how to set up dynamic worker count scaling in Pathway Live Data Framework'
---

# Dynamic Worker Scaling

Pipelines built with Pathway Live Data Framework allow you to scale workload and split heavy pipelines into parallel units of execution. This enables effective parallelization and higher throughput for computationally intensive workloads.
...
Dynamic scaling is not available when the worker pool is defined via the `--addresses` flag. In that mode, the set of processes is fixed for the entire run: Pathway Live Data Framework cannot add or remove machines at runtime. If your pipeline is launched with `--addresses`, scaling signals from workers are ignored and a warning is emitted to the logs. To use dynamic scaling, let Pathway Live Data Framework manage the processes itself by using `--processes` instead.

### License Limitations

You need a Pathway Live Data Framework Scale license in order for the scaling to work. You can obtain your free Pathway Live Data Framework Scale license [here](/framework/get-license). The page contains instructions for getting the license and using it in the pipeline.
