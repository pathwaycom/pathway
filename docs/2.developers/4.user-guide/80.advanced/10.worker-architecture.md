---
title: Architecture
description: 'This page explains the architecture of the Pathway Live Data Framework'
---

# Pathway Live Data Framework Architecture
...
Pathway was publicly announced at [PyData Global](/framework/blog/pydata) in December 2022, and entered a private beta stage.
...
The Pathway Live Data Framework uses Python to conveniently describe the computations to be performed.
The computations are then optimized and converted into basic operations executed by the Rust dataflow engine.

## Worker architecture

By design, each worker runs the same dataflow on different subsets (shards, partitions) of the data.
...
Some sources without explicit partitions can still be read in parallel: the filesystem connector, for instance, assigns each file to exactly one worker using a stable hash of its path, so reading scales with the number of workers.
For the remaining non-partitioned data sources, a single worker is responsible for reading the data and forwarding parts of it to other workers.
...
This is important for consistency: every result produced by Pathway Live Data Framework depends on a known prefix of the input data stream.

The basic dataflow design concepts for Pathway Live Data Framework follow those laid out in the foundational work of Microsoft Naiad ([SOSP 2013 paper](https://sigops.org/s/conferences/sosp/2013/papers/p439-murray.pdf)). The communication primitives, concepts of time within the system, and in-memory state representation are based on those of [Timely + Differential Dataflow](https://github.com/TimelyDataflow/differential-dataflow).
...
The following diagram sketches a multi-worker Pathway Live Data Framework deployment.
...
![Multi-worker Pathway Live Data Framework deployment diagram](assets/content/worker_architecture.png)

## Distributed deployment

A multi-server (distributed) deployment can use Kubernetes and its cloud implementations (AKS, EKS).
The Pathway Live Data Framework assumes a stateful set deployment with all pods present for a successful operation.
The Pathway Live Data Framework Enterprise covers distributed multi-server deployment for production use.
