...
description: 'This page explains the best practices for Pathway Live Data Framework pipelines'
---

# Best Practices for Efficient Pathway Live Data Framework Pipelines

To make the most of Pathway Live Data Framework, follow these best practices to optimize the performance and the memory usage of your streaming pipelines.

:article-toc-without-title

## Use built-in transformations instead of Python UDFs
The Pathway Live Data Framework provides a rich set of [built-in transformations](/developers/user-guide/data-transformation/table-operations) that are optimized for performance.
...
This helps the framework determine the result type in advance, optimizing execution and preventing unnecessary type inference overhead.
...
This allows the framework not to store results, thus reducing memory usage.
...
Since Pathway Live Data Framework operates on **incremental data updates**, prefer [stateless transformations](/developers/user-guide/introduction/concepts#stateful-and-stateless-transformations) when possible.
...
To limit memory usage in **windows and temporal joins**, make use of the [**temporal behaviors**](/developers/user-guide/temporal-data/windows_with_behaviors) provided by Pathway Live Data Framework.
...
Joins in Pathway Live Data Framework store both tables in memory, which can lead to high memory consumption.
If you only need to join entries from one table with the **current state** of another, use [`asof_now_join`](/developers/user-guide/data-transformation/indexes#asof-now-join), which stores only one side of the join.
...
You can start your pipeline with multiple worker processes using (this specific command starts 8 workers):
...
Wrapping transformations in functions and annotating arguments and results with `table: pw.Table[YourSchema]` boosts code readability.
