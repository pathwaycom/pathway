...
description: 'Tutorial about caching in Pathway Live Data Framework: how to set up and use memoization in AsyncTransformer'
date: '2023-11-30'
thumbnail: ''
...
# Setting up AsyncTransformer Cache in Pathway Live Data Framework
...
In Pathway Live Data Framework, it is possible by using [`AsyncTransformer`](/developers/user-guide/data-transformation/asynchronous-transformations), which has caching enabled.
...
Now, let's implement the Pathway Live Data Framework program that reads the data, applies the transformation, and outputs it so that you can check the result.

The cache is configured with the persistence config. It can be enabled by specifying the persistence mode parameter as `pw.PersistenceMode.PERSISTING` or `pw.PersistenceMode.UDF_CACHING`. In the first case, the framework will also persist in the internal state. In the second, you will only have the cache for the `AsyncTransformer` invocations, which is computationally cheaper.
