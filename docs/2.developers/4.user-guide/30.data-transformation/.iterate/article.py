...
# Pathway Live Data Framework allows you to perform iterative computations on tables. The tables can be updated over time but the data is still split into batches properly after performing an iteration.
#
# The iteration is performed until convergence or until the limit of steps is reached (by default there is no limit but you can specify it). The iteration is incremental - the next iteration step only gets elements that have been updated in the previous step. It allows Pathway Live Data Framework to reduce the amount of computations performed and effectively have a different number of iteration steps for different elements.
#
...
# In the sections below you can find the example applications of iteration in Pathway Live Data Framework with a more detailed explanation of how this mechanism works.
...
#
# You will now learn how to use Pathway Live Data Framework to test the Collatz conjecture. Let's import Pathway Live Data Framework first and define a function that performs a single step of the iterative computation. It can return a single table, a tuple or a dictionary of tables.
# %%
...
#
# If you uncomment the `t.debug("collatz")` line, you can analyze the flow of the data. You can see that the maximal number of steps was $10$ (for $n=13$) and the minimal number of steps was $8$ (for $n=3$). While counting steps, also a final step that transforms 1 to 1 was included. It is a necessary step that allows Pathway Live Data Framework to say that nothing has changed. The `pw.iterate` can only stop if there is no change in some iteration. If there was no change in one iteration, there won't be changes in any further iterations (functions passed to `iterate` should be deterministic to avoid problems).
...
# You can simulate a stream by adding `__time__` and `__diff__` columns to [`table_from_markdown`](/developers/api-docs/debug#pathway.debug.table_from_markdown). The `__time__` column simulates the time the record arrives to Pathway Live Data Framework and the `__diff__` determines whether the record is inserted (`+1`, the default) or deleted (`-1`).
...
# %% [markdown]
# ## Summary
# In this tutorial, you have learned how the iteration works in Pathway Live Data Framework and what are its applications. It can be useful in problems you encounter as well!
