...
description: 'Python Web Scraping with Pathway Live Data Framework'
date: '2025-03-13'
...
# Making Your Python Web Scraping Go Live with Pathway Live Data Framework
...
Fortunately, the Pathway Live Data Framework allows you to transform your existing web scraping scripts into efficient, real-time data processing pipelines, ensuring you always have up-to-date content.

In this tutorial, you will learn how to integrate your Python web scraper in the Pathway Live Data Framework to fetch the content on the websites you want frequently.
We provide a simple scraper for news websites as an example. The goal of this tutorial is to show you how to adapt and use your web scraper with the Pathway Live Data Framework, to have real-time web scraping.

Once ingested in real-time, the Pathway Live Data Framework allows you to filter, process, and transform the scraped content. You can use LLMs to extract topics, filter content, and notify stakeholders via Slack when new articles are scraped.
...
The repository includes both the web scraper we provide as an example, the Pathway Live Data Framework pipeline using it, and a ready-to-use Docker configuration to run the project.
...
- The constraints your Python web scraping script should follow in order to use it with the Pathway Live Data Framework.
- How to build a connector that uses the scraping script and dumps the results into a `jsonl` file. 

### Important Note about Python Web Scraping
Web scraping is difficult, and you can encounter different issues unrelated to the Pathway Live Data Framework when doing it on many websites for long periods of time. 
Some of these are:
...
## Ingesting The Live Scraping in Pathway Live Data Framework
Now, we can create a [custom Python connector](/developers/user-guide/connect/connectors/custom-python-connectors) to ingest the articles generated from the real-time generator.
...
Here, the connector will scrape the websites and create a table with the content. It will use the above generator to fetch the articles. This time, the engine will run the connector.
...
Scraping the web in real-time is a challenging task. The Pathway Live Data Framework helps you by providing a framework to build powerful real-time data processing pipelines.

In this tutorial, you learned how to build a connector that scrapes the web to fetch content from websites and dump it into a `jsonl` file.
Provided scraper is a simple example that can be adapted to your specific needs. The Pathway Live Data Framework also allows you to filter, process, and transform the scraped content in real-time, as the data arrives. This let's you build powerful applications that can be also supplemented with LLMs or ML models.

For example, you can send the scraped articles to a messaging service such as Slack, see [here](/developers/user-guide/connect/connectors/slack_send_alerts). Variety of connectors enable powerful workflows, you can find the full list of connectors [here](/developers/user-guide/connect/live-data-framework-connectors).
