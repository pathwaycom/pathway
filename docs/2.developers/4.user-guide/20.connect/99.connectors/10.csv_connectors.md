...
The Pathway Live Data Framework provides a [`pw.io.csv`](/developers/api-docs/pathway-io/csv) module with connectors to read and write data streams using CSV files.
...
```
The resulting table `t` will contain all the values contained inside the CSV files, and will be automatically updated whenever a new CSV file is added.
...
The final version of our project contains two files: `realtime_sum.py` which processes the stream using Pathway Live Data Framework and `generating_stream.sh` which generates the streams.
