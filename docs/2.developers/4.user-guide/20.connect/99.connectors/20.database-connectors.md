...
Connect Pathway Live Data Framework on top of your PostgreSQL/Debezium database using [`pw.io.debezium.read`](/developers/api-docs/pathway-io/debezium#pathway.io.debezium.read) and [`pw.io.postgres.write`](/developers/api-docs/pathway-io/postgres#pathway.io.postgres.write).
<!-- Pathway Live Data Framework provides connectors to read and write data streams using databases such as PostgreSQL. -->
...
In this tutorial, you will learn how to set up an architecture using Pathway Live Data Framework on top of PostgreSQL/Debezium.
The aim is to show you how to use Pathway Live Data Framework to perform real time processing of the changes of your PostgreSQL database.

If you're using [NeonDB](https://neon.com/) (serverless, PostgreSQL-compatible), see our [NeonDB integration guide](/developers/user-guide/connect/connectors/neondb-connector), which uses Pathway's native PostgreSQL connector for both reading (including CDC) and writing.
...
To create our input stream from PostgreSQL, we will use [Debezium](https://debezium.io/) and its associated connector [`pw.io.debezium.read`](/developers/api-docs/pathway-io/debezium#pathway.io.debezium.read).
The output stream will be sent back to the PostgreSQL database using the PostgreSQL connector [`pw.io.postgres.write`](/developers/api-docs/pathway-io/postgres#pathway.io.postgres.write).
...
You can do it as follows in Pathway Live Data Framework:
...
You need a PostgreSQL database with two tables: the tables `values` on which regular updates will be performed, creating the input data stream, and the `sum_table` output in which the sum will be stored and periodically updated by Pathway Live Data Framework.
...
The updates from Debezium are sent to Kafka, which relies on ZooKeeper, which then propagates them to Pathway Live Data Framework.

Finally, Pathway Live Data Framework receives the updates from Kafka, processes the data, and sends the sum back to PostgreSQL.
...
Now you need to set up the container for Pathway Live Data Framework.
...
You just need to use the pip install command to install the framework.
...
Each received update is atomic, and triggers the update of the pipeline created in Pathway Live Data Framework.
...
- `autocommit_duration_ms`: the maximum time between two commits. Every `autocommit_duration_ms` milliseconds, the updates received by the connector are committed and pushed into the dataflow.
...
- `table`: the table to send to PostgreSQL,
...
⚠️ The table should already be created in PostgreSQL: the creation is not done by Pathway Live Data Framework.
...
To learn more about what those updates mean, don't hesitate to read the [explanation provided in our first example](/developers/user-guide/introduction/first-realtime-app#understanding-the-output).
