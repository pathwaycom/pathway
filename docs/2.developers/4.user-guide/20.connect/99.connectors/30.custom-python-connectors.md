...
In this tutorial, you will learn how to create a Python connector that will allow you to connect to your custom data source and feed data directly into Pathway Live Data Framework.

In order to create a custom connector, you need to extend the [`pw.io.python.ConnectorSubject`](/developers/api-docs/pathway-io/python#pathway.io.python.ConnectorSubject) class and implement the `run` method, which is responsible for reading the source and feeding the data into the buffer.  

`ConnectorSubject` serves as a bridge between the data source and the engine. It comes with a special method [`next`](/developers/api-docs/pathway-io/python#pathway.io.python.ConnectorSubject.next) allowing you to push data into the buffer.
...
## Polling an external API: upsert sessions

A very common connector shape is *polling*: periodically fetch the full current state of an
external source - a REST endpoint listing projects, a database table, a directory - and send
everything you see. Written naively, such a connector crashes on the second poll: by default every
`next` call is an *insertion* of a new row, and sending a row whose primary key already exists is
an error.

For this pattern, construct the subject with `session_type="upsert"`. Then a row sent with an
already-present primary key **replaces** the previous version, and rows that disappeared from the
source can be removed with `delete`. Upsert sessions require the schema to define a primary key -
this is what the row identity is derived from.
...
class InputSchema(pw.Schema):
    project_id: str = pw.column_definition(primary_key=True)
    name: str
    status: str

class ProjectsSubject(pw.io.python.ConnectorSubject):
    ...
    def run(self):
        ...
        while True:
            current = {row["project_id"]: row for row in fetch_projects()}
            for row in current.values():
                self.next(**row)  # new and changed rows become upserts
            for project_id, row in previous.items():
                if project_id not in current:
                    self.delete(**row)  # rows gone from the source are removed
            previous = current
            self.commit()
            time.sleep(self.poll_interval_s)

table = pw.io.python.read(ProjectsSubject(), schema=InputSchema)
```

Each poll cycle ends with `commit`, so downstream consumers see one consistent snapshot per cycle.
Unchanged rows may be re-sent as is: an upsert with identical values is a no-op for the result
(though skipping them, as the `previous` dict makes easy, saves processing).
...
### Embedded methods
- [`next`](/developers/api-docs/pathway-io/python#pathway.io.python.ConnectorSubject.next): takes values as keyword arguments. Values for all columns of the table should be passed to this method. The values are sent as a single row to the engine.
- [`next_bytes`](/developers/api-docs/pathway-io/python#pathway.io.python.ConnectorSubject.next_bytes): sends `bytes` to the engine in `data` column. If you want to have `bytes` in a different column/multiple columns, just use `next`.
- [`next_str`](/developers/api-docs/pathway-io/python#pathway.io.python.ConnectorSubject.next_str): sends `string` to the engine in `data` column. If you want to have `string` in a different column/multiple columns, just use `next`.
- [`commit`](/developers/api-docs/pathway-io/python#pathway.io.python.ConnectorSubject.commit): sends a commit message to the engine. Only when the data is committed, the engine starts processing it. You can commit manually using this method or rely on autocommits that can be set in [`pw.io.python.read`](/developers/api-docs/pathway-io/python#pathway.io.python.read).
...
- `autocommit_duration_ms`: the maximum time between two commits. Every `autocommit_duration_ms` milliseconds, the updates received by the connector are committed automatically and pushed into the dataflow.
