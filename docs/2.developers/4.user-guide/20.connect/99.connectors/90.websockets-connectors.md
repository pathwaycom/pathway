...
description: 'Creating a custom WebSockets connector in Pathway Live Data Framework'
date: '2023-01-24'
...
Let's now define a [schema](/developers/user-guide/connect/schema) that describes the structure of the resulting table. It will correspond to the incoming data from the API since the incoming payload is not modified.
...
Now, all that's left is to run the pipeline with [`pw.run`](/developers/api-docs/pathway#pathway.run):
