...
# %% [markdown]
# # Basic Data Types
# This guide is about basic data types in Pathway Live Data Framework: it covers the list of basic data types that can be used in Pathway Live Data Framework, explores several available conversion methods, and wraps up with examples of operators that require a column of specific data type as input.

# Currently, Pathway Live Data Framework allows using the following basic Python types: `bool`, `str`, `int`, `float`, and `bytes`. Additionally, there is support for types `datetime` and `duration` from `datetime` module, distinguishing between `utc datetimes` and `naive datetimes`. Finally, Pathway Live Data Framework also introduces an additional type for columns representing pointers, called `Pointer`. Below, you can find an example table with six columns, one example column for each of the basic Python types, and one column of type Pointer. The complex types (as datetime) need some conversion, and they are covered in [the later part of the article](/developers/user-guide/connect/datatypes#temporal-data-types).
# The standard way to define a type in Pathway Live Data Framework is to use a [`schema`](/developers/api-docs/pathway#pathway.Schema) (you can learn more about schemas in this [article](/developers/user-guide/connect/schema)):
# %%
import datetime
...
# %% [markdown]
# ## Implicit Typing
# By default, you don't need to worry about they types of columns created with `select` or `with_columns`. The expressions used in those operators have defined output type, Pathway Live Data Framework knows it, and assigns the types of new columns automatically.
...
# %% [markdown]
# Similarly, the special columns produced by some of the Pathway Live Data Framework operators (examples in the later part of the article) have fixed types and as such, you don't need to bother with the types of those columns.
...
# Pathway Live Data Framework supports operations on Python `int` and `float` types, and on their `numpy` counterparts. Below, you can find a few short examples that read and convert numbers in Pathway Live Data Framework.
#
# ### Type `int`
# %%
...
# %% [markdown]
# ### Numerical Module
# In case you need to use some basic operations on columns of numerical type, Pathway Live Data Framework provides a [module](/developers/api-docs/pathway#pathway.internals.expressions.NumericalNamespace) containing functions over numerical data types such as [`abs`](/developers/api-docs/pathway#pathway.internals.expressions.NumericalNamespace.abs) or [`round`](/developers/api-docs/pathway#pathway.internals.expressions.NumericalNamespace.round).

# %% [markdown]
# ## Temporal Data Types
# In Pathway Live Data Framework, temporal data types (`datetime.datetime`) are complex data types with some representation as some simple type (as `int` or `str`). As such, you first need to load the input as simple type, and only then convert it to temporal type.
# Similarly to Python, Pathway Live Data Framework distinguishes between [naive datetime](/developers/api-docs/pathway#pathway.DateTimeNaive) (not aware of timezones) and [UTC datetime](/developers/api-docs/pathway#pathway.DateTimeUtc) (aware of time zones).
# Below, you can find examples of reading both kinds of datetime, initially provided as `str` and `int`, using methods from the Pathway Live Data Framework [`dt` module](/developers/api-docs/pathway#pathway.internals.expressions.DateTimeNamespace):
