...
description: 'How to define the behavior of your streamed data in the Pathway Live Data Framework'
---


# Understanding Temporal Behavior in Pathway Live Data Framework

When moving from batch to streaming you need to understand the nature of data coming over time. the Pathway Live Data Framework makes it easy to switch from batch to streaming by guaranteeing out-of-the-box the correctness of computations, by updating a result whenever new data points are inputted. This correctness comes with a price, and you may want to trade it for better latency or memory consumption. In the Pathway Live Data Framework, you can set the behavior of the temporal operations to determine the tradeoff between accuracy, latency, and memory consumption that will suit your application.
...
The default behavior of the Pathway Live Data Framework is to start the computation right away and wait for potential late data to ensure correctness: by default, the results produced are always consistent with the received data.
However, this has a cost: the Pathway Live Data Framework needs to keep track of old data in case a late data point comes and the result needs to be recalculated.

For example, if you compute the exact number of unique website visitors over 1-hour intervals and get an entry from 1 day ago, you must remember all users from that time. It is okay, but imagine getting an entry from 1 year ago and keeping all the data! Doing this might be ridiculous, but it is the only way to guarantee the correctness if the maximal latency is unknown. Unfortunately, this can make the memory consumption unbounded. The solution is to specify a temporal behavior to let the Pathway Live Data Framework know which data is essential for you and which can be ignored.
...
All these issues can be solved in the Pathway Live Data Framework by using temporal behaviors! They will allow you to specify when computations should occur, whether to keep updating based on late data, or if you want to remove outdated results from the output of an operator.

## Defining temporal behaviors in Pathway Live Data Framework

Temporal behavior in the Pathway Live Data Framework is specified using 3 parameters - `delay`, `cutoff` and `keep_results`.
...
`cutoff` is used to specify how long to wait for late data. It sets the time, after which the results of computation results will no longer be updated, even if late data arrives, thus allowing the framework to clear memory.
