...
# This article explores concepts related to the temporal behavior of Pathway Live Data Framework's interval join, like filtering out
...
#
# Let's consider a scenario in which you track customers ordering products. Each order is placed at some specific time (`order_time`) and reaches the tracking system written in Pathway Live Data Framework at time `__time__`.
...
# To make that trade-off possible, the Pathway Live Data Framework provides the `behavior` parameter for `interval_join`, which defines its temporal behavior. Roughly speaking, it allows you to tell the Pathway Live Data Framework to ignore the records that are too late.  Subsequently, that allows you to forget the records that you know won't be joined with any new incoming records in the future.
# To be more precise: if you set the `behavior` to e.g. `pw.temporal.common_behavior(cutoff=6)`, the Pathway Live Data Framework will ignore all records that have times less or equal to maximal already seen time minus $6$. Small remark: the maximal already seen time is held separately for each side of the join, as it allows expressing a join with historical data slightly easier.
...
# or equal to $128-6=122$ had to be ignored. Note that Min's order came to the Pathway Live Data Framework at
...
# display (or even store) old orders. Don't worry! The Pathway Live Data Framework can solve this problem too.
...
#
# In this example, a special column `__diff__` is used to tell the Pathway Live Data Framework whether a record
