---
title: 'Pathway Live Data Framework MCP Server'
description: 'Tutorial about how to set up and use Pathway Live Data Framework MCP Server'
date: '2025-08-01'
thumbnail: ''
...
# Pathway Live Data Framework MCP Server
...
The Pathway Live Data Framework provides its own MCP Server to allow you to deliver real-time statistics and document indexing for your agentic applications.
In this article, you will learn how to set up the MCP Server.
...
The MCP Server provides the real-time data processing capabilities of the Pathway Live Data Framework engine to your AI applications.
...
You need to install Pathway Live Data Framework LLM xpack to use the MCP server: `pip install pathway[xpack-llm]`
...
- **Real-Time Statistics**: The MCP Server can provide real-time statistics to LLMs, enabling them to make informed, data-driven decisions based on the latest information.
...
It is done by using an instance of a class inheriting the **`McpServable`** class.
...
It must have two parameters: `self` and a table, let's call it `input_from_client` (it can be named the way you want).
The table `input_from_client` follows the `EmptyRequestSchema` schema and contains a single row.
...
Once your tool is defined, you can easily set up an MCP server using the **`PathwayMcp`** class:
...
The MCP server requires the following arguments:
...
The interest of the Pathway Live Data Framework is to be able to work on real-time tables.
...
You can now easily access basic statistics about a table.
Those numbers will evolve with the table, making sure your MCP client works on fresh data!


## Exposing Pathway Live Data Framework Document Store
...
The MCP Server allows you to expose its document store to MCP clients.
...
The [`DocumentStore`](/developers/api-docs/pathway-xpacks-llm/document_store) inherits from `McpServable` and can be passed directly to `PathwayMcp`.
...
The MCP Server provides real-time data and context to your AI applications.
...
As real-time data becomes more important, tools like the MCP Server will be essential for effective data handling and decision-making.
