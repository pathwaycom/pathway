...
description: "Create your own RAG Pipeline with Pathway Live Data Framework"
navigation: true
date: '2025-03-28'
thumbnail: ''
...
# Create your own real-time RAG with Pathway Live Data Framework

In this guide, you will learn how to construct a dynamic, real-time RAG App using Pathway Live Data Framework and OpenAI.
...
The Pathway Live Data Framework allows you to easily index in real-time your documents so your RAG is always up-to-date.
...
You need to ingest them into the pipeline using [Pathway Live Data Framework connectors](/developers/user-guide/connect/connectors-overview).
...
```
You can find the list of Pathway Live Data Framework connectors [here](/developers/user-guide/connect/live-data-framework-connectors).


Now, you need to index those documents.
This can be easily done in Pathway Live Data Framework using a [DocumentStore](/developers/api-docs/pathway-xpacks-llm/document_store) which stores and manages the documents and their embeddings.
...
You can use the [`OpenAIEmbedder`](/developers/api-docs/pathway-xpacks-llm/embedders#pathway.xpacks.llm.embedders.OpenAIEmbedder) to use OpenAI Embedding services.
...
You can learn more about document indexing in Pathway Live Data Framework in our [dedicated article](/developers/user-guide/llm-xpack/docs-indexing).
...
You also need to define the input schema, using [pw.Schema](/developers/user-guide/connect/schema), which helps to enforce the structure of the data being processed by Pathway Live Data Framework.
...
Once everything is ready, you can create the server using the [REST connector](/developers/api-docs/pathway-io/http#pathway.io.http.rest_connector).
...
Now users can send queries to Pathway Live Data Framework via HTTP requests.
...
The Pathway Live Data Framework provides [wrappers](/developers/user-guide/llm-xpack/llm-chats) for most of the LLM providers.
...
No worries! The Pathway Live Data Framework automatically adapts to changes, ensuring your RAG stays updated in real time.

With Pathway Live Data Framework, static and real-time data are treated the same way: your pipeline is already working on data stream!

Once your documents are preprocessed and indexed, Pathway Live Data Framework automatically detects any changes in the document directory and updates the document store accordingly.
...
## Conclusions and Pathway Live Data Framework wrappers
In this tutorial, you have learned to create a RAG pipeline from scratch with Pathway Live Data Framework: you have implemented the pipeline step-by-step.
The RAG is live by default, updating the index whenever the documentation changes.

In addition to our [ready-to-run templates](/developers/templates?tab=ai-pipelines), Pathway Live Data Framework provides pre-build wrapper to use [pre-selected prompts](/developers/api-docs/pathway-xpacks-llm/prompts), [servers](/developers/api-docs/pathway-xpacks-llm/servers), or even entire [RAG pipeline](/developers/api-docs/pathway-xpacks-llm/question_answering).
