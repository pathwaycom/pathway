...
You can now start data wrangling. The first thing to do is to load the data from the CSV file into a Pathway Live Data Framework table, using `pw.io.csv.read`. In general, at the time you define the logic of our Pathway Live Data Framework script, the data is not available yet. As such, the graph representing Pathway Live Data Framework computation is typed, just to be sure it works properly on data streamed in the future. For this reason, the `pw.io.csv.read` function needs a schema which is a blueprint for the data structure.
