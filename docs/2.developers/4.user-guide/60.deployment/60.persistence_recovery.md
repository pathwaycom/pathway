---
title: 'Persistence in Pathway Live Data Framework: How to Resume the Interrupted Computation'
description: 'Tutorial on persistence in Pathway Live Data Framework: how to set it up and how to use it to recover the interrupted computation'
...
# Persistence in Pathway Live Data Framework: Resume the Interrupted Computation

In this article, you will learn how to resume the interrupted computation. We will start with a fairly simple Pathway Live Data Framework application, which doesn't persist in its state.

It will be run as a separate process in the streaming mode, that is, waiting for the inputs all the time.

In another process, we will run the streamer: a simple script that puts the data into the input directory every so often. So, we will expect that the Pathway Live Data Framework program fetches the data from the streamer and uses it in the computation.

We will then terminate the computation and show that when being re-run, the application without persistence indeed starts and provides the results from the very beginning.
...
Now let's create a simple Pathway Live Data Framework program that would do the word count for us. For now, we don't try to be persistent.
...
Last but not least point, we simulate the environment where the application is run as a separate process - as it usually does. To do so we save the code into a separate file named `wordcount.py` and then use `subprocess.Popen` class to run it.
...
Right after launching the streaming, we launched our Pathway Live Data Framework program. We wait for 5 seconds, which is not enough for the streamer to produce all input blocks and then we terminate it with the kill method of `subprocess.Popen`. And then we wait for another five seconds for the streamer to complete.
...
The main idea behind this is that the framework periodically dumps the state to the given data storage backend. When being restarted, the framework first looks for the snapshot of the data that was made and if it finds this snapshot, the snapshot gets loaded into the engine and it doesn't have to do the reads and the processing for the data that was preserved there.
...
In principle, this assignment can be done automatically by the engine. In this case, it will assign unique names to the sources in the order of their appearance and construction. However, this is not generally recommended if you need to change your Pathway Live Data Framework program and the data source in the future.
...
Now we can apply the described changes to the program that we had in the beginning. We still need a way to run it in a separate process that can be interrupted, so the interface stays the same. At the same time, we have the following persistent code saved as the wordcount application:
