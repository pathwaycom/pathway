...
article:
  ...
  tags: ['showcase', 'llm']
keywords: ['LLM', 'RAG', 'GPT', 'OpenAI', 'Google Docs', 'Slack', 'alert', 'KNN', 'HTTP connector', 'Streamlit', 'unstructured']
---

# Use LLMs for notifications: crafting a RAG app with real-time alerting

How to get real-time alerts with changes in Google Docs using [Pathway](https://pathway.com/).
## 
...
* We need to execute `python [app.py](&lt;http://app.py>)` , follow the instructions in[ Running the project](https://github.com/pathwaycom/llm-app/tree/main/examples/pipelines/drive_alert/README.md#run-the-project) to get the app up and ready!
