...
"""Identity pipeline used by the NATS JetStream persistence tests.

Reads plaintext messages from a JetStream-backed NATS topic and appends them
to a jsonlines file, with filesystem persistence enabled. Run as a subprocess
so that the harness can SIGKILL it at an arbitrary moment and restart it
...
def main():
    ...
    table = pw.io.nats.read(
        ...
        args.topic,
        format="plaintext",
        jetstream_stream_name=args.stream,
