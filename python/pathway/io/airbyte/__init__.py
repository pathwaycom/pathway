...
from pathway.io._utils import DurationLike, _get_unique_name, as_duration_seconds
...
def read(
    ...
    *,
    ...
    enforce_method: str | None = None,
    dependency_overrides: list[str] | None = None,
    refresh_interval: DurationLike = 60,
    name: str | None = None,
...
mode. Please note that reusage Airbyte license is not supported at the moment.
    ...
    If the local execution type is selected, Pathway Live Data Framework initially attempts to find the
    ...
    Args:
        config_file_path: Path to the config file, created with
            ...
            or just via Pathway Live Data Framework CLI (that uses ``airbyte-serverless`` under the hood). The "source"
        ...
        mode: denotes how the engine polls the new data from the source. Currently
            ...
            it will check for updates every ``refresh_interval`` seconds. ``"static"``
        ...
        refresh_interval: time between new data queries, given as a number of
            seconds or a ``datetime.timedelta`` / ``pw.Duration``. Applicable
            if mode is set to ``"streaming"``.
        enforce_method: when set to ``"docker"``, the Pathway Live Data Framework will not try to locate and
            run the latest connector version from PyPI. On the other hand, when set to
            ``"pypi"``, the Pathway Live Data Framework will prefer the usage of the latest image available on
    ...
    To do that, you can use Pathway Live Data Framework CLI command ``airbyte create-source``. You can create
...
`Airbyte GitHub connector <https://docs.airbyte.com/integrations/sources/github>`_.
    ...
    Now, you can simply configure the Pathway Live Data Framework connector and run:
    ...
    small values for ``refresh_interval`` is not advised for the remote runs, as they may
