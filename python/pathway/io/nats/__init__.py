...
def read(
    ...
    * For the ``"raw"`` format, the payload is read as raw bytes and added directly to the
    table.
    * In the ``"plaintext"`` format, the payload decoded from UTF-8 and stored as plain text.
    ...
    As a result, you will have a table with three columns: ``"id"``, ``"username"``, and
    ``"phone"``. The ``"id"`` column will also act as the primary key for the Pathway table.
    """
...
def write(
    ...
    *,
    ...
    corresponding to the processing time of the entry and ``pathway_diff`` that is either 1 or -1.
    Both header values are provided as UTF-8 encoded strings. If ``headers`` parameter is used,
    ...
    done by providing ``key`` and ``value`` parameters. In order to output extra values
    from the table in these formats, NATS headers can be used. You can specify the column
    references in the ``headers`` parameter, which leads to serializing the extracted fields
