...
LISTINGS = [
    ...
    {
        "listing_id": 1006,
        "title": "Renovated 1BR Near Metro",
        "price": "1800.00",
        "location": "District 4",
        "source": "website_a",
...
async def main():
    ...
    for listing in LISTINGS:
        await producer.send(
            "listings_raw", AMQPMessage(body=json.dumps(listing).encode())
        )
        await asyncio.sleep(0.2)
    ...
    while True:
        ...
        await producer.send(
            "orders_raw", AMQPMessage(body=json.dumps(new_order).encode())
        )
        ...
        await producer.send(
            "listings_raw", AMQPMessage(body=json.dumps(new_listing).encode())
        )
