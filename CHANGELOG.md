...
### Changed
- Dependency version requirements were refreshed where it matters most, moving to the newest robust, bug-free, and secure releases: `deltalake` moved from the 0.17 line to 1.6+ (bringing numerous correctness and performance improvements on the write path), `pdfminer.six` is pinned to the 20260107 release, `litellm` now requires 1.84+ (1.83.0 on Python 3.14) together with the OpenAI SDK 2.x it depends on, `llama-index-core` now requires 0.13+, and the `transformers` upper bound was lifted from 4.49 to the latest 4.x. The `tests` extra now uses pytest 9. User-visible API of Pathway itself is unchanged.
