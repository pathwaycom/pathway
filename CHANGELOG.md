...
### Added
- The Apache Pulsar connectors now integrate with the broker's schema registry through the new `"avro"` format. `pw.io.pulsar.write(format="avro")` derives an Avro schema from the table, registers it in the registry and publishes the rows as Avro binary — readable by any schema-aware Pulsar consumer. `pw.io.pulsar.read(format="avro")` resolves the writer schema of every message in the registry by the version stamped into the message metadata, so a topic whose schema evolved stays readable, including the messages written under its older schema versions.
- `pw.io.pulsar.read` with the `"avro"` and `"json"` formats no longer requires the `schema` parameter: when it is omitted, the columns of the table are deduced from the schema the topic carries in the broker's registry, at pipeline construction time. An explicit schema always overrides the deduction.
- `pw.io.pulsar.read` now supports reading the compacted view of a topic: the `read_compacted` parameter delivers only the latest message per partition key for the compacted part of the topic. Requires the `"exclusive"` or `"failover"` subscription type.
- `pw.io.pulsar.write` now accepts a `producer_name` parameter that names the producers on the broker (with a per-worker suffix), making the pipeline identifiable in the topic stats.
- `pw.io.pulsar.read` now supports `with_metadata=True`, which adds a `_metadata` column describing the Pulsar message every row came from: the topic, the partition, the message id components, the publish and event timestamps, the producer name, the ordering key and the user-defined properties.
- `pw.io.pulsar.write` now accepts an `event_time` parameter setting the native `event_time` field of the produced messages: either a column of the table (an integer in milliseconds since the UNIX epoch, or a UTC datetime), or the new `pw.io.ENGINE_TIME` marker that uses the engine (minibatch) time of the update — the same value the messages carry in the `pathway_time` property.
- `pw.io.pulsar.write` now accepts an `ordering_key` parameter — a column whose value becomes the ordering key of the produced messages, used by `key_shared` subscriptions to keep the messages of one entity in order independently of the partition key.
- `pw.io.pulsar.read` with the `"raw"` and `"plaintext"` formats now exposes the partition key of each message in the `key` column, next to the payload in `data`. As in the Kafka connector, the connector first tries to use the partition key as the row's primary key and autogenerates one for the messages published without a partition key; the new `autogenerate_key` parameter switches to autogenerated primary keys for every message.
- `pw.io.pulsar.write` can now compress the produced messages: the `compression` parameter accepts `"lz4"`, `"zlib"` and `"zstd"`. `pw.io.pulsar.read` decompresses the incoming messages of these codecs automatically, whichever client produced them. Snappy is not supported.
